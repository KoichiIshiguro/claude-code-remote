...
- ⚡ **セッション毎に `claude` を常駐** — 初回プロンプトでキャッシュを温め、以降のターンは同じプロセスを使い続けるので Anthropic のプロンプトキャッシュが温いまま。会話が伸びてもトークン消費が爆発しない
...
- **セッション毎に `claude` を 1 プロセス、常駐。** 初回プロンプトで起動し、以降のプロンプトは JSON 行として stdin に流し込む。プロセスはセッションが生きている限り生存。Anthropic のプロンプトキャッシュが温いまま使えるので、会話が伸びてもトークン消費が膨らみにくい。プロセスが死んでも（クラッシュ・サーバ再起動）、次のプロンプトで `--resume <session_id>` 付きで再生成 — 真の会話状態は Claude 本体の `~/.claude/projects/*.jsonl` にある。
