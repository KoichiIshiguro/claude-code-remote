...
スマホからは `http://100.101.102.103:4040`、または MagicDNS で
`http://<PC のマシン名>:4040` でアクセスできます（マシン名は Tailscale アプリや
...
`http://localhost:4040` を開く → `/setup` に自動リダイレクトされる初回ウィザードでユーザー名・パスワード・アクセススコープ（サンドボックスする単一フォルダ、または信頼できる tailnet 上でのフルアクセス）を設定すれば完了。

セットアップ後、スマホからは `http://<Tailscale-IP>:4040` でアクセス（ウィザード画面に URL と QR コードが表示されます）。
...
### 5.（強く推奨）サーバーに長期認証トークンを渡す

このサーバーは本物の `claude` CLI をヘッドレス `-p` モードで動かします — **プロンプト
毎に `claude` を 1 プロセス**起動します。各プロセスは、Claude がディスク／ログイン
Keychain に保存した OAuth 認証情報で認証しますが、**この保存済み認証情報は時間とともに
腐ります**：短命のアクセストークンは期限切れになり、保存されたリフレッシュトークンも
やがてローテーションで無効化されサーバーに拒否されます。こうなると毎プロンプトが次で
失敗します：
...
…**自分の GUI マシンで手動起動していても、です。** ややこしいのは、その間も *対話版* の
`claude` TUI は動き続けること — 起動しっぱなしのセッションは起動時にメモリ上でトークンを
更新し、保存先には書き戻さないためです。なので「TUI は普通にログインできてる」ことは、
サーバーの `-p` 呼び出しが通ることを **意味しません**。（launchd/systemd 常駐サービスは
さらに悪条件で、GUI ログインセッションの外で動くため Keychain を一切読めません。）

確実な解決策は、**どんな運用でも**、保存済み認証情報に依存しない**長期トークン**を使う
ことです。マシン上で、実際のターミナルから（ブラウザが開くので TTY が必要）：

```bash
claude setup-token        # ブラウザで認可。sk-ant-oat01-... が表示される
```

表示されたトークンをこのディレクトリの `.env` に入れ、サーバーを（再）起動します：
...
`.env` は gitignore 済みなのでトークンはこのマシンに留まります。サーバーは spawn する
各 `claude` にこれをそのまま渡すので、認証が（腐っているかもしれない）Keychain のコピーに
依存しなくなります。トークン単体での動作確認：

```bash
CLAUDE_CODE_OAUTH_TOKEN=sk-ant-oat01-... claude -p "reply with: ok"
```

`ok` が返ればサーバーも認証できます。トークンは長期間有効で、失効させるか認証情報を
ローテーションしない限り再取得は不要です。
...
> **⚠️ 常駐サービスには[ステップ 5](#5強く推奨サーバーに長期認証トークンを渡す)の長期トークンが*必須*。** launchd/systemd の常駐エージェントは GUI ログインセッションの *外* で動くため Keychain を一切読めません。`.env` に `CLAUDE_CODE_OAUTH_TOKEN` が無いと毎プロンプトが `401 Invalid authentication credentials` で失敗します。サービス登録の **前に**、ステップ 5 の手順でトークンを設定してください。
...
- **プロンプトキューはブラウザではなくサーバー側。** 応答ストリーム中に送ったプロンプトはセッション毎のキューに溜まり、ランナーがブラウザ非依存で順次消化する（タブを閉じても実行される）。ライブキューはメモリ上（再起動でクリア＝仕様）。**予約送信**はその永続版で、`data/scheduled-prompts.json` に保存する。期限が来た予約は、ランナーが各ターンの先頭で**ライブキューより優先して**取り込むため、ターン実行中に予約時刻を過ぎても、そのターン完了の瞬間にまとめて発火する（30 秒間隔のタイマーはアイドルなセッションを*起こす*だけで、自身が発火させることはない＝ターン中に取りこぼし／孤立しない）。
...
| 変数 | 説明 |
|---|---|
| `PORT` | HTTP ポート。デフォルト `4040` |
| `CLAUDE_PATH` | `claude` の絶対パス — PATH が継承されない環境（PM2 / systemd / launchd）で必要 |
| `CLAUDE_CODE_OAUTH_TOKEN` | `claude setup-token` で得る長期認証トークン。**サーバー運用全般で強く推奨**（保存済みの Keychain／ディスクの OAuth 認証情報は腐り、`claude -p` が自分のマシンでも 401 になる）。**launchd/systemd 配下では必須**（Keychain を一切読めない）。上記ステップ 5 参照 |
