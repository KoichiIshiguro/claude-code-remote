...
- ⏰ **予約送信（send-later）** — Gmail 風の分割送信ボタン。本体はこれまで通り即送信、`▾` でプロンプトを指定時刻に予約。予約は**サーバー側**で発火するのでブラウザを閉じても・再起動しても実行される。`session limit · resets …` のリセット時刻を自動検出するプリセットもあり、待たせておいた続きが自動で再開する
